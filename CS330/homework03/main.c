...
{	
    ...
    *csr_vals = (double*)malloc(nnz * sizeof(double));
    ...
    // Compute histogram of row indices
    for (int i = 0; i < nnz; i++) {
        hist[row_ind[i]]++;
    }

    // Compute prefix sum of histogram 4  row pointers
    (*csr_row_ptr)[0] = 0;
    for (int i = 1; i <= m; i++) {
        (*csr_row_ptr)[i] = (*csr_row_ptr)[i-1] + hist[i-1];
    }

    // Copy the COO entries into the CSR arrays
    for (int i = 0; i < nnz; i++) {
        int row = row_ind[i];
