...
{ 
  ...
  printf("\nOriginal matrix(print by row):\n");
  apply_by_row(print_str, data, data_rows, data_cols);
  printf("\n");

  //btw print behaves differently depending if its calle by row or by column
  printf("\nOriginal matrix (print by col):\n");
 ...
 */
void
apply_by_row(void (*func)(char*, int, int, int, int), char*** data, int rows, int cols)
{
  int i, j;
  for (i = 0; i < rows; i++)
  {
...
capitalize_first_col(char* in_str, int row, int col, int rows, int cols)
{
  ...
  {
  in_str[i] = toupper((unsigned char)in_str[i]);
  }  
}
 ...
 */
void
apply_by_col(void (*func)(char*, int, int, int, int), char*** data, int rows, int cols)
{
