...
namespace Mem {
  ...
  public:
    ...
    // TODO: Add overloaded operator declarations and / or friend statements here
  	...
  	IntQueue & operator<<(int new_value){Enqueue(new_value); return *this;};
    	IntQueue & operator>>(int & store_to){store_to = Dequeue(); return *this;};
  };

  // TODO: Add prototypes for any of IntQueue's friends here
  std::ostream & operator<<(std::ostream &s, Mem::IntQueue &q);
...
// done: Add overloaded operator implementations here

std::ostream &
Mem::operator<<(std::ostream &s, Mem::IntQueue &q)
{
	// loop and print out elements in queue
	Mem::Elem<int> *next = q.head;
       	while(next != NULL)
	{
		s<<next;
		next = next->getNext();
