...
main(int argc, char **argv)
{
  	...
  	for (i = 0; i < size; i++) 
  	{
		//this was initially not allocated right, should have been size of int 
    		num[i] = (int *)malloc(size * sizeof(int));
  	}

  	// Set values
  	// the error is that there should be two loops here
  	for (i=0; i<size; i++){
  		for (j = 0; j < size; j++) 
  		{	
    			num[i][j] = i * size + j;
  	...
  	// Deallocate memory
	// memory was not freed properly, internal arrays needed to be freed	
  	for (i = 0; i < size; i++) 
  	{
    		free(num[i]);
	}
