 ...
 */
void
init_adj_list(adj_node_t*** list, int rows)
{
    // Allocate memory
 ...
 */
void
add_node(adj_node_t** list, int row, adj_node_t* node)
{
    if(list[row] == NULL) {
 ...
 */
void
construct_adj_list(int** adj_mat, int rows, int cols, adj_node_t*** list)
{
	...
	int i,j;
	// adj list, it should only have nodes that have values
	for(i=0; i<rows; i++)
	{
		for(j=0; j<cols; j++)
		{	
			// see if node is non-zero
			if (adj_mat[i][j] != 0)
			{
				// add node takes care of cases where the node
				// is the first adjacent node to be added
				// so we can just call it in all non-zero cases
 ...
 */
void 
bfs(adj_node_t** list, int rows, int source, int* color, int* distance, int* parent)
{
	...
	// queue is 1d, therefore the list is "1 row"
	adj_node_t** queue;
	init_adj_list(&queue, 1);
	
	// initialized the queue, adding the source vertex
	add_node(queue, 0, create_node(source));
	distance[source] = 0;

    	// bfs iteration
	while(*queue != NULL)	{
		int cur_index = queue[0]->vid;	
		adj_node_t * cur_node = list[cur_index];
		// checking adj nodes to the current head of queue
		while(cur_node != NULL)
		{	
			int deck_id = cur_node->vid;
			// orphan case (undiscovered)
			if (color[deck_id] == 0)	{
				// adopt the node
				parent[deck_id] = cur_index;

				// update its distance
				distance[deck_id] = distance[parent[deck_id]] + 1;

				// put it in the queue, change its color to grey (1)
				color[deck_id] = 1;
				add_node(queue, 0, create_node(deck_id));
			}
			// need to update to next adj node no mater what
			cur_node = cur_node->next;
		}
		// deque the head node, update its color to black (2)
		color[cur_index] = 2;
		remove_node(queue);		
	}
   	fprintf(stdout, "done\n");
