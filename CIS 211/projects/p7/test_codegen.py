...
def crush(text: Union[str, List[str]]) -> List[str]:
    ...
    if isinstance(text, str):
        ...
        lines = text
    squished = [squish(i) for i in lines]
    crushed = [i for i in squished if len(i) > 0]
    return crushed
...
class Test_If_Gen(AsmTestCase):
    ...
    def test_if_gen(self):
        context = Context()
        target = context.allocate_register()
        e = If(EQ(Var("x"), IntConst(1)), Assign(Var("y"), Minus(Var("x"), IntConst(1))), Assign(Var("x"), IntConst(2)))
        e.gen(context, target)
        expected = """
        LOAD  r14,var_x
        LOAD  r13,const_1
