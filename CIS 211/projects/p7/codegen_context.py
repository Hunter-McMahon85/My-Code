...
class Context(object):
    ...
    def __init__(self):
        # A table of integer constants to be declared at the end of the source program.
        # The table maps values to names, so that we can reuse them.
        self.consts = {}

        # A table of variables to be declared at the end of the source program,
        # with the symbols used for them in the assembly code.
        self.vars = {}

        # Instructions in the source code, as a list of strings.
        self.assm_lines = []
    ...
    def get_lines(self) -> List[str]:
        ...
        for const_val in sorted(self.consts):
            code.append(f"        {self.consts[const_val]}: DATA {const_val}")
        for i in self.vars:
            code.append(f"        var_{i}: DATA {0}")
        return code
