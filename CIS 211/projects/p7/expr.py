...
class Plus(BinOp):
    ...
    @staticmethod
    def _apply(left: int, right: int) -> int:
        return left + right
...
class Read(Expr):
    ...
    @staticmethod
    def gen(context: Context, target: str):
        """We print by storing to the memory-mapped address 511"""
        context.add_line(f"       LOAD {target},r0,r0[511]")
...
class If(Control):
    ...
    def gen(self, context: Context, target: str):
        """
        Generate code for the if statements
        """
        else_label = context.new_label("else")
        fi_label = context.new_label("fi")
        if self.cond == 1:
            self.cond.condjump(context, target, context.new_label("if"))
            ...
            self.cond.condjump(context, target, else_label)
            self.thenpart.gen(context, target)
            context.add_line(f"        JUMP  {fi_label}")
            context.add_line(f"        {else_label}:")
            self.elsepart.gen(context, target)
