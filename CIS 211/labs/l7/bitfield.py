...
log = logging.getLogger('bitfield.py')
...
class BitField(object):
    ...
    def __init__(self, from_bit: int, to_bit: int) -> None:
        ...
        Args:
            from_bit: the starting bit (0 is the right-most)
            to_bit: the ending bit
        """
    ...
    def extract(self, word: int) -> int:
        ...
        Args:
            word: the integer from which to extract bits
        """
        word_bit = bin(word)
        end_bit = None
        if self.from_bit == 0:
            end_bit = len(word_bit)
        else:
            end_bit = -self.from_bit
        taken_bits = "0b" + word_bit[-self.to_bit - 1:end_bit]
        log.debug(taken_bits)
        return int(taken_bits, 2)
    ...
    def insert(self, value: int, word: int) -> int:
        ...
        Args:
            value: the integer into which to insert value
            word: the value to insert
        Returns:
            The new integer after insertion
        Example:
            BitField(3,5).insert(0b101, 0b110) == 0b101110
        """
        new_value = word | (value & self.mask << self.from_bit)
        log.debug(bin(new_value))
        return new_value

    def extract_signed(self, word: int) -> int:
        """Extract bits in bitfield as a signed integer.
        Args:
            word: the signed integer from which to extract bits"""
        base_word = self.extract(word)
        signed_word = sign_extend(base_word, self.to_bit - self.from_bit + 1)
        log.debug(base_word)
        log.debug("bw")
        log.debug(signed_word)
        log.debug("sw")
        return signed_word
